...
const baseUri = '/api/requests';

async function acceptRequest(val, answer) {
    let message = answer;
    if (!message || message === '') {
        message = 'Uw verzoek tot verificatie werd geaccepteerd door onze administrators.';
    }
    ...
    try {
        ...
        toastr.clear();
        toastr.success('Het request is succesvol geaccepteerd.');
    } catch (err) {
        toastr.clear();
        toastr.error('Er ging iets mis bij het accepteren van het request, sorry.');
    }
}
...
async function denyRequest(val, answer) {
    let message = answer;
    if (!message || message === '') {
        message = 'Sorry maar uw verzoek tot verificatie werd geweigerd door onze administrators.';
    }
    ...
    try {
        ...
        treated.innerText = `Behandeld? ${true}`;
        toastr.clear();
        toastr.success('Het request is succesvol afgekeurd.')

    } catch (err) {
        toastr.error('Er ging iets mis bij het afkeuren, sorry.');
    }
}
...
async function getRequests() {
    try {
        ...
        toastr.clear();
        toastr.error('Er ging iets mis bij het inladen van de requests, sorry.');
        return [];
    }
}
...
async function loadRequests() {
    ...
    requests.forEach((request) => {
        ...
        table.id = `Table${request.requestId}`;
        ...
        requestIdColumn.innerText = `VerzoeksID: ${request.requestId}`;
        rowHeading.appendChild(requestIdColumn);
        const usernameColumn = document.createElement('td');
        usernameColumn.id = 'UserID';
        usernameColumn.innerText = `Gebruikersnaam: ${request.userName}`;
        ...
        reasonColumn.innerText = `Reden voor verificatie: ${request.reason}`;
        ...
        if (!request.treated) {
            ...
            answer1.addEventListener('click', () => {
                acceptRequest(request.requestId, text.value);
            });
            ...
            answer2.addEventListener('click', () => {
                denyRequest(request.requestId, text.value);
            });
        ...
        dateColumn.id = 'Date';
        dateColumn.innerText = "Datum verzoek"+request.date;
...
function addEventHandlers() {
    ...
    $('#Search').keyup(() => {
        ...
        $('.Search').each(() => {
            // nog uit te zoeken hoe ik kan zoeken op secondChild
            if (!$(this.childNodes[0].childNodes[1]).text().toLowerCase().includes(searchText)) {
                $(this).hide();
            } else {
                $(this).show();
            }
